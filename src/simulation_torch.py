...
def build_random_adj_matrix(size, D, plot_deg_dist=False):
    ...
    '''
    connection_matrix = torch.diag(torch.ones(size-1), diagonal=1)
    for i in range(size-2):
        connection_matrix[i, i+2:] = (torch.rand(size-i-2) < (D-2)/(size-2))
...
class Simulation:
    ...
    def __init__(self, pop_size, D, base_transmission_prob, time_infected, counter_measure_params= default_counter_measure_parameters):
        ...
        self.social_dist_mask = torch.rand(self.pop_size,self.pop_size) > counter_measure_params["soc_dist_effect"]
        ...
        self.is_suscep = torch.ones(self.pop_size)
        # indicates if a node is infected (1) or not (0)
        self.is_infected = torch.zeros(self.pop_size)
        # stores for how many days node remains infected; 0 for uninfected
        self.infected_for = torch.zeros(self.pop_size)
    ...
    def step(self, n=1, disable_progress_bar=True, steps_after_halt=2):
        ...
        for i in tqdm(range(n), desc="running simulation", disable=disable_progress_bar):
            ...
            eff_connection_matrix = self.connection_matrix.clone()
            if self.D() != self.base_D:
                eff_connection_matrix = eff_connection_matrix * self.social_dist_mask

            pot_infections = (torch.matmul(eff_connection_matrix,self.is_infected)*self.is_suscep).int()
            infections = torch.zeros(self.pop_size)
            for i in torch.where(pot_infections != 0)[0]:
                if (torch.rand(pot_infections[i].item()) < self.transmission_prob()).any():
                    infections[i] = 1
            ...
            self.infected_for = torch.clip(self.infected_for-1, 0, None)
            self.is_infected[self.infected_for == 0] = 0

            if torch.sum(self.is_infected) == 0:
                halt_counter += 1
    ...
    def randomize_states(self):
        ...
        self.is_infected = torch.randint(0, 2, self.pop_size)
        self.is_suscep = torch.clip(np.random.randint(0, 2, self.pop_size)-self.is_infected, 0, None)
        self.infected_for = torch.zeros(self.pop_size)
        self.infected_for[self.is_infected == 1] = torch.randint(1, self.time_infected+1, np.sum(self.is_infected))
